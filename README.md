...
手动同步可选择 `all`（全部有效商品）、`pending`（未发布商品）或 `updated`（内容标记为更新且尚无商品/素材记录），定时同步也会保存其中一种范围。同步数据池与看板“有效游戏数据”复用同一条件：标题、简介、有效 `http/https` 图片链接及至少一个有效 `http/https` 下载地址完整；没有下载记录、空地址、非网页协议资源或缺少图片的记录会在任务创建前过滤。素材导入和商品发布使用两条独立异步队列，并行数可在任务页面分别配置为 `1–12`；单件素材准备完成后立即进入单商品发布队列，不再等待整组素材，也不再调用批量发布接口。页面分别展示素材和发布的分段进度：本轮成功为绿色、已有数据跳过为黄色、失败为红色，并以本轮有效游戏总量为分母。已有闲鱼商品编号或已有发布编号的游戏在候选阶段直接跳过；已有本地素材编号且内容未变化的游戏只跳过素材上传，并复用该素材进入发布队列；内容哈希变化时仍会更新素材库。远程素材库存在同名标题时仍跳过，不重复创建或发布。封面从模板结果下载，经过重试、图片解码和 JPEG 标准化后保存到持久卷；已有素材进入单商品发布时也复用该稳定封面地址。单商品发布调用同步执行期间每 1 分钟记录一次等待心跳，明确失败或结果未知均只记录当前商品并跳过，继续处理后续商品。发布成功后立即使用接口返回的闲鱼商品编号关联卡券 ID `6`，并把商品编号和链接写回对应 `games` 记录；卡券关联失败只重试关联，不会重复发布商品。

采集和同步任务会把账号校验、网络读取、跳过判定、详情保存、封面缓存、素材写入、单商品发布、发布等待心跳和卡券关联等步骤写入持久化日志。看板当前任务和任务记录均可打开详细日志弹窗；运行中的日志每 2 秒自动追加，历史任务也可随时回看。旧版本已经结束的任务不会补写详细操作日志。
...
curl -X PUT \
  -H 'Content-Type: application/json' \
  -H 'X-API-Key: <闲鱼用户API Key>' \
  -d '{"cron_timezone":"Asia/Shanghai","crawl":{"cron_schedule":"0 3 * * *","enabled":true,"concurrency":3},"sync":{"cron_schedule":"0 */6 * * *","enabled":true,"mode":"all","material_concurrency":4,"publish_concurrency":4}}' \

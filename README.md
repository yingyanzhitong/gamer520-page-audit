...
手动同步可选择 `all`（全部有效商品）、`pending`（未在当前账号成功发布或卡券关联待重试）或 `updated`（已发布但素材发生变化），定时同步也会保存其中一种范围。同步按每 20 个候选商品循环处理：先以最多 4 个并行请求完成当前 20 个商品的素材库同步，随后立即发布这一组，再继续导入下一组；第一组待发布商品仍先提交 1 条探针，探针确认成功后发布组内剩余商品。页面分别展示“导入素材库进度”和“发布商品进度”。同名标题仍按顺序检查，避免并发创建重复素材。封面在素材同步阶段从模板结果下载，经过重试、图片解码和 JPEG 标准化后保存到持久卷，闲鱼发布器读取 Gamer520 自身的稳定封面地址。每个发布批次前后都会刷新当前账号商品列表，通过新增商品编号和标题前缀核对实际结果；页面未跳转但商品已出现时会纠正为成功，成功文案没有商品编号且列表也无法核验时则停止并等待确认。探针失败、账号异常、同批多条页面或图片失败、批次失败率过高时会安全熔断后续发布，并且不会继续导入后续工作组。发布成功后会逐件关联卡券 ID `6`。素材库已有同名商品时跳过；“已更新商品”在同步前已经是已发布状态，同步时只更新素材库，不进入发布批次，同步后仍保持发布成功且不会重新上架，线上闲鱼商品内容也不会自动修改。发布成功后，闲鱼商品编号和链接会写回对应 `games` 记录；卡券关联失败只重试关联，不会重复发布商品。

...
- `xianyu_sync_settings`：持久化所有发布账号共用的默认售价、库存、发布方式以及标题、简介、图片模板；`xianyu_account_settings` 仅保留用于兼容旧版本数据。

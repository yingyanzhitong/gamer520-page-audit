...
游戏列表的采集状态包含“成功、已更新、缺失、失败、违规”；缺少图片或下载资源、或图片链接无法访问时记为“缺失”。“违规”仅能由后台手动标记或手动恢复为成功，用于闲鱼认定违规的商品；采集任务不会自动改写该状态，且违规游戏不会进入任何发布候选列表。仅采集成功的游戏可单独同步。第一列只展示游戏标题和热度，不再显示有效性备注。闲鱼状态筛选按页面当前保存的发布账号统一分为“无、加入素材库、发布中、发布成功”，两个筛选条件可以组合使用。
...
curl -X PUT \
  -H 'Content-Type: application/json' \
  -H 'X-API-Key: <闲鱼用户API Key>' \
  -d '{"cron_timezone":"Asia/Shanghai","crawl":{"cron_schedule":"0 3 * * *","enabled":true,"concurrency":3},"sync":{"cron_schedule":"0 */6 * * *","enabled":true,"mode":"all","material_concurrency":4,"publish_batch_size":8}}' \

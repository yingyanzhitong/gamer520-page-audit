...
游戏列表的采集状态筛选包含“已更新”，用于查看最近一次内容变更类型为更新的商品；闲鱼状态筛选按页面当前保存的发布账号区分待同步、素材已同步、已发布、发布失败等状态，两个筛选条件可以组合使用。
...
手动同步可选择 `all`（全部有效商品）、`pending`（未在当前账号成功发布或卡券关联待重试）或 `updated`（已发布但素材发生变化），定时同步也会保存其中一种范围。素材同步最多同时执行 4 个请求，成功且尚未发布的素材累计到 20 个后提交一次批量发布，最后不足 20 个的尾批也会提交；同名标题仍按顺序检查，避免并发创建重复素材。发布成功后会逐件关联卡券 ID `6`，页面实时展示当前商品与完成进度。标题、简介、封面与卡券标题都使用当前模板的渲染结果；图片结果必须是 `http/https` URL。素材库已有同名商品时跳过；“已更新商品”在同步前已经是已发布状态，同步时只更新素材库，不进入发布批次，同步后仍保持“已发布”且不会重新上架，线上闲鱼商品内容也不会自动修改。发布成功后，闲鱼商品编号和链接会写回对应 `games` 记录；卡券关联失败只重试关联，不会重复发布商品。
